...
import serial  # importiere pyserial, um mit dem Arduino zu kommunizieren
...
def take_photo_and_print_poem():
    ...
    # Send print command to Arduino to trigger the print action
    send_print_command()
...
##########################
# Function to send print command to Arduino
##########################
def send_print_command():
    # Seriellen Port und Baudrate für den Arduino festlegen
    arduino = serial.Serial('/dev/ttyUSB0', 9600)  # Ändere den Port nach Bedarf

    # Sende den Druckbefehl an den Arduino
    if arduino.is_open:
        arduino.write(b'PRINT_COMMAND')  # Beispielbefehl an den Arduino senden
        print("Druckbefehl an Arduino gesendet.")
    else:
        print("Serielle Verbindung zum Arduino nicht geöffnet.")
